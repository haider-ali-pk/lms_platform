...
import { stripe, STUDENT_PLAN_PRICES } from "@/app/lib/stripe";
...
export async function POST(req: NextRequest) {
  ...
  // Only reuse customer ID if it's a real Stripe customer (starts with cus_)
  let customerId: string;
  if (existing?.stripe_customer_id?.startsWith("cus_")) {
    customerId = existing.stripe_customer_id;
  } else {
    // Always create a new real Stripe customer
    ...
    // Update the DB with the real customer ID immediately
    if (existing) {
